...
# from openrlhf.train.cli.free_port_utils import find_free_port
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--advantage_estimator",
        type=str,
        choices=["gae", "reinforce", "rloo", "reinforce_baseline", "group_norm", "dr_grpo", "opo", "dpg", "reinforce_global", "reinforce_baseline_nomean"],
        default="gae",
    ...
    parser.add_argument("--use_global_token_level_loss", action="store_true", default=False)

    # apply-seq-balancing
    parser.add_argument("--use_seq_balancing", action="store_true", default=False)

    # apply-dynamic-batch-size
    parser.add_argument("--use_dynamic_bz", action="store_true", default=False)

    # dpg_beta
    parser.add_argument("--dpg_beta", type=float, default=1.0, help="Temperature for evaluation")

    # repeatness_threshold
    parser.add_argument("--use_filter_sample", action="store_true", default=False)

    parser.add_argument("--use_adv_mask_before", action="store_true", default=False)
    parser.add_argument("--use_adv_mask_after", action="store_true", default=False)
    parser.add_argument("--use_adv_mask_skip", action="store_true", default=False)

    # repeatness_threshold
    parser.add_argument("--use_cispo", action="store_true", default=False)

    # use-adv-whitening
    parser.add_argument("--remove_advantage_whiten", action="store_true", default=False)

    # exact-on-policy
    parser.add_argument("--drop_samples", action="store_true", default=False)

    # sample-strategy
    parser.add_argument("--sample_strategy", type=str, default='acc_balancing')

    # positive-gamma
    parser.add_argument("--use_positive_gamma", action="store_true", default=False)

    # use_adv_shaping
    parser.add_argument("--use_adv_shaping", action="store_true", default=False)

    # entropy_shape_alpha
    parser.add_argument("--entropy_shape_alpha", type=float, default=0.1, help="Temperature for evaluation")

    # entropy_shape_kappa
    parser.add_argument("--entropy_shape_kappa", type=float, default=2.0, help="Temperature for evaluation")

    # use_adv_shaping
    parser.add_argument("--use_reward_smooth", action="store_true", default=False)

    # use_length_acc_balance
    parser.add_argument("--use_length_acc_balance", action="store_true", default=False)

    # use_exp_norm_by_length
    parser.add_argument("--use_exp_norm_by_length", action="store_true", default=False)

    # use_exp_before_queue
    parser.add_argument("--use_exp_before_queue", action="store_true", default=False)

    # use_exp_before_queue
    parser.add_argument("--use_dual_policy_loss", action="store_true", default=False)

    # use_loss_mask
    parser.add_argument("--use_loss_mask", action="store_true", default=False)

    # use_mopps
    parser.add_argument("--use_mopps", action="store_true", default=False)

    # use_outlier_mask
    parser.add_argument("--use_outlier_mask", action="store_true", default=False)

    # use_dynamic_bs_ds
    parser.add_argument("--use_dynamic_bs_ds", action="store_true", default=False)

    # reward-key
    parser.add_argument("--reward_key", type=str, default='reward', help="TensorBoard logging path")

    # advantge-clip
    parser.add_argument("--advantage_clip_low_high", type=float, nargs=2, default=None, help="PPO-clip low and high")
    ...
    if args.advantage_estimator in ["rloo", "reinforce_baseline", "group_norm", "reinforce_baseline_nomean"]:
