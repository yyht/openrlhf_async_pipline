...
def masked_mean(tensor: torch.Tensor, mask: Optional[torch.Tensor], env_mask: Optional[torch.Tensor], dim: int = None) -> torch.Tensor:
    # if mask is None:
    ...
    if env_mask is None:
        ...
        return (tensor * env_mask).sum(axis=dim) / env_mask.sum(axis=dim)

def masked_sum(tensor: torch.Tensor, mask: Optional[torch.Tensor], env_mask: Optional[torch.Tensor], dim: int = None) -> torch.Tensor:
    # if mask is None:
    #     return tensor.sum(axis=dim)
    ...
    if env_mask is None:
        ...
        return (tensor * env_mask).sum(axis=dim)
