...
from openrlhf.cli.template import TEMPLATE_DICT
import os, logging
from openrlhf.trainer.ray.async_vllm_engine_async_server import create_async_vllm_engines

logging.basicConfig()
logger = logging.getLogger(__name__)
logger.setLevel(logging.INFO)

TEMPLATE_NAME = os.getenv('TEMPLATE_NAME', 'ZERO_V1')
...
def train(args):
    ...
    if args.vllm_num_engines is not None and args.vllm_num_engines > 0:
        ...
        vllm_engines = create_async_vllm_engines(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--eps_clip_high", type=float, default=0.28, help="PPO clip range")
    parser.add_argument("--value_clip", type=float, default=0.2, help="PPO value clip range")
    parser.add_argument("--value_clip_high", type=float, default=0.28, help="PPO value clip range")
    ...
    parser.add_argument(
        "--advantage_estimator",
        type=str,
        choices=["gae", "reinforce", "rloo", "reinforce_baseline", "group_norm", "control_variate", "control_variate_f_div"],
        default="gae",
        help="Choose advantage estimation method: gae, reinforce, rloo, reinforce_baseline, group_norm, control_variate, control_variate_f_div",
    )
    ...
    # use-acc-filter
    parser.add_argument("--use_acc_filter", action="store_true", default=False)
    # use-global-shuffle
    parser.add_argument("--global_shuffle", action="store_true", default=False)
    # add data-level-filter
    parser.add_argument("--use_data_filter", action="store_true", default=False)
    # add entropy_loss_coef
    parser.add_argument("--entropy_loss_coef", type=float, default=0, help="Entropy loss")
    # sft-loss sft_loss_ratio
    parser.add_argument("--sft_loss_coef", type=float, default=0.0, help="Ring attention group size")
    # add dpg_ratio
    parser.add_argument("--dpg_beta", type=float, default=1.0, help="Entropy loss")
    # add data-level-filter
    parser.add_argument("--normalize_mean", action="store_true", default=False)
    # use_prefetch
    parser.add_argument("--use_prefetch", action="store_true", default=False)
    # use_critic_prefetch
    parser.add_argument("--use_critic_prefetch", action="store_true", default=False)
    # reuse_offline
    parser.add_argument("--reuse_offline", action="store_true", default=False)
    # use_env_mask
    parser.add_argument("--use_env_mask", action="store_true", default=False)
    # dump_data
    parser.add_argument("--dump_data", action="store_true", default=False)
    # use_eval_filter
    parser.add_argument("--use_eval_filter", action="store_true", default=False)
    # use-length-filter
    parser.add_argument("--use_length_filter", action="store_true", default=False)
    # use_critic_advantage_normalize
    parser.add_argument("--use_critic_advantage_normalize", action="store_true", default=False)
