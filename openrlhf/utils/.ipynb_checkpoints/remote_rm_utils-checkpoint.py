...
def request_api_wrapper(url, data, score_key="rewards", try_max_times=30):
    ...
    random.seed(seed)
    url_base = random.randint(0, 100000000)
    
    for try_idx in range(try_max_times):
        url_idx = url_base+try_idx
        url = url_list[np.mod(url_idx, len(url_list))]
        try:
