...
class DeepspeedStrategy(ABC):
    ...
    def backward(self, loss: torch.Tensor, model: nn.Module, optimizer: optim.Optimizer, **kwargs) -> None:
        if isinstance(model, Actor):
            model = model.model
        if kwargs.get('scale_by_manual', False):
            model.backward(loss, scale_wrt_gas=False)
        else:
            model.backward(loss)

    def get_global_norm(self, model: nn.Module):
        if isinstance(model, Actor):
            model = model.model
    ...
    def get_ds_train_config(self, is_actor):
        ...
        if self.args.use_dynamic_bs_ds:
            ...
            logger.info(
                {
                    'INFO': f"using_dynamic_batch_size",
                    'VALUE': ds_config["train_micro_batch_size_per_gpu"]
                }
            )
        else:
