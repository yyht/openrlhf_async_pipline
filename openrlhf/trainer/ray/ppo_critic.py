...
class CriticPPOTrainer(PPOTrainer):
    def ppo_train(self):
        ...
        logger.info({
            'INFO': '##TRAIN_CRITIC_LOADER##',
            'STATUS': len(dataloader)
        })
        ...
        for epoch in range(self.max_epochs):
            ...
            for experience in pbar:
                if self.strategy.args.use_critic_prefetch:
                    ...
                    if len(prefetch) % self.strategy.accumulated_gradient == 0:
                        ...
                        length_status = {
                            'response_length': prefetch[0].info['response_length'].float().sum()
                        }
                        ...
                        for (local_step, exp) in enumerate(prefetch):
                            ...
                            status = self.training_step(exp, length_status=length_status)
                            # for DP
    ...
    def training_step(self, experience: Experience, length_status=None) -> Dict[str, float]:
        return self.training_step_critic(experience, length_status=length_status)
...
class CriticModelRayActor(BasePPORole):
    ...
    def replay_buffer_reset(self, actor_replay_buffer):
        # acc-filter or global_shuffle
        logger.info({
            'INFO': '##TRAIN_CRITIC_BUFFER_LOCAL##',
            'STATUS': len(self.trainer.replay_buffer),
            'ACTOR_BUFFER_SIZE': len(actor_replay_buffer),
        })
        self.trainer.replay_buffer.clear()
        logger.info({
            'INFO': '##TRAIN_CRITIC_BUFFER_CLEAR##',
            'STATUS': len(self.trainer.replay_buffer)
        })
        self.trainer.replay_buffer.items.extend(actor_replay_buffer.items)
        logger.info({
            'INFO': '##TRAIN_CRITIC_BUFFER_ACTOR##',
            'STATUS': len(self.trainer.replay_buffer)
        })
        self.trainer.replay_buffer.gathered = actor_replay_buffer.gathered
