...
class CriticPPOTrainer(ABC):
    ...
    def training_step(self, experience: Experience, step: int, local_step=None, accumulation_steps=None) -> Dict[str, float]:
        self.critic.train()
...
class CriticModelRayActor(BasePPORole):
    ...
    def save_checkpoint(self, tag):
        ...
        if args.save_hf_ckpt:
            save_path = os.path.join(args.ckpt_path, f"{tag}_hf_critic")
            self.strategy.save_model(
                self.critic,
                None,
                save_path,
