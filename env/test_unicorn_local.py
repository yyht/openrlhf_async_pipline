...
class ServerController:
    def __init__(self):
        ...
        self.max_batch_size = 8  # 单个批次最大请求数
    ...
    #             response = await self.generate_sync(request.prompts, sampling_params)
    ...
    async def add_requests(self, url, prompts, prompt_ids, sampling_params, **kwargs):
        requests = self.build_requests(prompts, prompt_ids, sampling_params)
        ...
        for start_idx, batch in batches:
            responses_ray.append(process_batch_requests.remote(url, start_idx, batch, **kwargs))
...
class RayActor():
    def __init__(self):
        self.async_llm_workers = [ServerController.remote()]
...
for _ in range(1):
    ...
    refs = ray_actor.async_llm_workers[0].add_offline_requests.remote([prompt]*128, 
    [prompt_token_ids]*128, sampling_params, process_fn=None,
    tokenizer=tokenizer)

    outputs = ray.get(refs)

    print(len(outputs), '===local===', time.time()-start)
    json.dump(outputs, open('/cpfs/debug/outputs_local.json', 'w'))
